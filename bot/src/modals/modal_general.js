...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const category = await resolveTicketCategory(interaction, 'general');
    if (!category) return;
    ...
    const { results: [tSubject, tDesc], anyTranslated } = await translateToEnglish([subject, description]);

    const displaySubject = tSubject.text;
    const displayDesc    = tDesc.text;

    const ch = await interaction.guild.channels.create({
      name: `〚${ticketNum}〛『general-support』`,
      parent: category.id,
    ...
    await Ticket.create({
      ...
      username: interaction.user.tag, category: 'general',
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        ...
        `${e('target') || '🔍'}  **Reason:** \`General Support\`\n` +
    ...
    const formEmbed = new EmbedBuilder()
      ...
      .addFields(
        { name: `${e('note') || '📝'}  Subject`,     value: fieldValue(tSubject), inline: false },
        { name: `${e('info') || 'ℹ️'}  Description`, value: fieldValue(tDesc),    inline: false },
        { name: `${e('screenshot') || '📸'}  Evidence`, value: evidence ? `[View Attachment](${evidence.url})` : '`Not provided`', inline: true },
      )
      .setFooter({ text: `UPCore  •  General Support  •  User Submission${anyTranslated ? '  •  Auto-translated to English' : ''}` });
    ...
    // ── Forward evidence attachment if submitted ───────────────────────────────
    if (evidence) {
      const evidenceEmbed = new EmbedBuilder()
        .setColor(0x5865F2)
        .setTitle(`${e('screenshot') || '📸'}  Attached Evidence`)
        .setDescription(`${e('member') || '👤'}  Submitted by <@${interaction.user.id}>`)
        .setImage(evidence.url)
        .setFooter({ text: 'UPCore  •  General Support' });
      await ch.send({ embeds: [evidenceEmbed] });
    }

    await interaction.editReply({ content: `${e('success') || '✅'}  Your ticket has been opened: <#${ch.id}>` });
    ...
    await logTicketAction(client, 'TICKET_OPEN', {
      ...
      fields: [
        ...
        { name: `${e('log') || '🗂️'}  Category`,        value: '`General Support`',                                       inline: true  },
        ...
        { name: `${e('note') || '📝'}  Subject`,        value: `\`\`\`${subject.slice(0, 900)}\`\`\``,                   inline: false },
      ],
    });
  },
};
