...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const category = await resolveTicketCategory(interaction, 'business');
    if (!category) return;

    const platform  = interaction.fields.getTextInputValue('platform');
    const document  = interaction.fields.getUploadedFiles('document', false)?.first() ?? null;
    ...
    const { results: [tPlatform], anyTranslated } = await translateToEnglish([platform]);

    const displayPlatform = tPlatform.text;

    const ch = await interaction.guild.channels.create({
      name: `〚${ticketNum}〛『business-enquiries』`,
      parent: category.id,
    ...
    await Ticket.create({
      ...
      username: interaction.user.tag, category: 'business',
      originalCategoryId: category.id,
      status: 'open', openedAt,
      formData: {
        'Platform / Enquiry': platform,
        ...(document ? { 'Document / Media': document.url } : {}),
      },
    });
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        ...
        `${e('target') || '🔍'}  **Reason:** \`Business Enquiry\`\n` +
    ...
    const formEmbed = new EmbedBuilder()
      ...
      .addFields(
        { name: `${e('info') || 'ℹ️'}  Platform / Enquiry`, value: fieldValue(tPlatform), inline: false },
        { name: `${e('note') || '📄'}  Document / Media`, value: document ? `[View Attachment](${document.url})` : '`Not provided`', inline: true },
      )
      .setFooter({ text: `UPCore  •  Business Enquiry  •  User Submission${anyTranslated ? '  •  Auto-translated to English' : ''}` });
    ...
    // ── Forward document/media if submitted ───────────────────────────────────
    if (document) {
      const docEmbed = new EmbedBuilder()
        .setColor(0x5865F2)
        .setTitle(`${e('note') || '📎'}  Attached Document / Media`)
        .setDescription(`${e('member') || '👤'}  Submitted by <@${interaction.user.id}>`)
        .setImage(document.url)
        .setFooter({ text: 'UPCore  •  Business Enquiry' });
      await ch.send({ embeds: [docEmbed] });
    }

    await interaction.editReply({ content: `${e('success') || '✅'}  Your business enquiry has been opened: <#${ch.id}>` });
    ...
    await logTicketAction(client, 'TICKET_OPEN', {
      ...
      fields: [
        ...
        { name: `${e('log') || '🗂️'}  Category`,          value: '`Business Enquiries`',                                    inline: true  },
        { name: `${e('channel') || '#'}  Channel`,        value: `<#${ch.id}>`,                                             inline: true  },
        { name: `${e('calendar') || '📅'}  Opened At`,    value: `\`${openedAt}\``,                                         inline: true  },
        { name: `${e('screenshot') || '📸'}  Document`,   value: document ? `[View](${document.url})` : '`Not provided`',  inline: true  },
        { name: `${e('info') || 'ℹ️'}  Platform / Enquiry`, value: `\`\`\`${platform.slice(0, 900)}\`\`\``,               inline: false },
      ],
    });
  },
};
