...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const category = await resolveTicketCategory(interaction, 'club');
    if (!category) return;

    const trophies   = interaction.fields.getTextInputValue('trophies');
    const screenshot = interaction.fields.getUploadedFiles('profile_screenshot', false)?.first() ?? null;
    ...
    const ch = await interaction.guild.channels.create({
      name: `〚${ticketNum}〛『club-join-request』`,
      parent: category.id,
    ...
    await Ticket.create({
      ...
      username: interaction.user.tag, category: 'club',
      originalCategoryId: category.id,
      status: 'open', openedAt,
      formData: {
        'Total Trophies': trophies,
        ...(screenshot ? { 'Profile Screenshot': screenshot.url } : {}),
      },
    });
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        ...
        `${e('target') || '🔍'}  **Reason:** \`Club Join Request\`\n` +
    ...
    const formEmbed = new EmbedBuilder()
      ...
      .addFields(
        { name: `${e('star') || '🏆'}  Total Trophies`,          value: `\`${trophies}\``, inline: true },
        { name: `${e('screenshot') || '📸'}  Profile Screenshot`, value: screenshot ? `[View Attachment](${screenshot.url})` : '`Not provided`', inline: true },
      )
      .setFooter({ text: 'UPCore  •  Club Join Request  •  User Submission' });
    ...
    // ── Forward profile screenshot if submitted ───────────────────────────────
    if (screenshot) {
      const screenshotEmbed = new EmbedBuilder()
        .setColor(0x5865F2)
        .setTitle(`${e('screenshot') || '📸'}  Profile Screenshot`)
        .setDescription(
          `${e('member') || '👤'}  Submitted by <@${interaction.user.id}>\n` +
          `${e('star') || '⭐'}  Trophies: \`${trophies}\``
        )
        .setImage(screenshot.url)
        .setFooter({ text: 'UPCore  •  Club Join Request' });
      await ch.send({ embeds: [screenshotEmbed] });
    }

    await interaction.editReply({ content: `${e('success') || '✅'}  Your club join request has been opened: <#${ch.id}>` });
    ...
    await logTicketAction(client, 'TICKET_OPEN', {
      ...
      fields: [
        ...
        { name: `${e('log') || '🗂️'}  Category`,             value: '`Club Join Request`',                                      inline: true },
        { name: `${e('channel') || '#'}  Channel`,           value: `<#${ch.id}>`,                                              inline: true },
        { name: `${e('star') || '🏆'}  Total Trophies`,      value: `\`${trophies}\``,                                          inline: true },
        { name: `${e('calendar') || '📅'}  Opened At`,       value: `\`${openedAt}\``,                                          inline: true },
        { name: `${e('screenshot') || '📸'}  Screenshot`,    value: screenshot ? `[View](${screenshot.url})` : '`Not provided`', inline: true },
      ],
    });
  },
};
