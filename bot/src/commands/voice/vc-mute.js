...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption(o => o.setName('user').setDescription('The user to VC mute').setRequired(true))
  ...
  async execute(interaction, client) {
    ...
    if (member.voice.serverMute) {
      return interaction.editReply({
        embeds: [warn(`${e('warning')} **${member.displayName}** is already server muted.`)],
      });
    }

    await member.voice.setMute(true, `${reason} | Mod: ${interaction.user.tag}`);

    const newCase = await createCase(interaction.guildId, {
      action: 'VC_MUTE', userId: targetUser.id, userTag: targetUser.tag,
      moderatorId: interaction.user.id, moderatorTag: interaction.user.tag, reason,
    });

    await logModAction(client, interaction.guild, {
      action: 'VC_MUTE', target: targetUser.tag, targetId: targetUser.id,
    ...
    await interaction.editReply({
      embeds: [ok(`${e('check')} Voice muted **${member.displayName}** in \`${member.voice.channel.name}\` — Case \`#${String(newCase.caseId).padStart(4, '0')}\``)],
    });
  },
};
